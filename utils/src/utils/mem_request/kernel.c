...
t_kernel_mem_req *crear_kernel_mem_request(kernel_req_operation operacion, u_int32_t pid, u_int32_t tamanio, char *path)
{
    t_kernel_mem_req *mem_request = malloc(sizeof(t_kernel_mem_req));
    mem_request->operacion = operacion;
    mem_request->pid = pid;
    mem_request->tamanio = tamanio;
...
void enviar_kernel_mem_request(int32_t fd_memoria, t_kernel_mem_req *mem_request)
{
    ...
    agregar_a_paquete(paquete, &(mem_request->tamanio), sizeof(u_int32_t));
...
t_kernel_mem_req *recibir_kernel_mem_request(int32_t fd_kernel)
{
    ...
    mem_request->tamanio = *(u_int32_t *)list_get(paquete, 2);
